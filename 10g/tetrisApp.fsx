...
type tetromino (a:bool[,], c:Color, o:position) =
    ...
    member this.rotateRight() = 
        let h = this.height
        let w = this.width
        Array2D.init this.height this.width (fun i j -> this.image[w-1-j,i])
...
type t1()=
    inherit tetromino((Array2D.create 3 2 true), Red, (3,0))
    do
...
//Black box test af tetromino
let tb = tetromino((Array2D.create 2 2 true),Yellow,(4,0))
printfn "TB 1: %A" (tb.image = (Array2D.create 2 2 true))
printfn "TB 2: %A" (tb.col = Yellow)
printfn "TB 3: %A" (tb.offset = (4, 0))
printfn "TB 4: %A" (tb.height = 2)
printfn "TB 5: %A" (tb.width = 2)
printfn "TB 6: %A" (tb.rotateRight() = tb.image)

//Black box test af board
let tb2 = board(35,30)
printfn "TB 1: %A" (tb2.width = 35)
printfn "TB 2: %A" (tb2.height = 30)
printfn "TB 3: %A" (tb2.board = Array2D.create 35 30 None)
...
let tb2_np() = 
    ...
    match p with 
        |Some x ->
                a.put(x)
                if a.board = (Array2D.create 10 10 None) then
                    i <- false
                elif a.take() = p then
                    i <- true
                else 
...
printfn "TB 4: %A" (tb2_np())
printfn "TB 5: %A" (tb2.current = None)
printfn "TB 6: %A" (tb2.put(square()))
printfn "TB 7: %A" (tb2.take() = tb2.current)
printfn "TB 8: %A" (tb2.isValid(square(),(1,1)))

//White box test
let tw = tetromino((Array2D.create 2 2 true),Yellow,(4,0))
let wb_test_set_tetromino = [
    [tw.image, tw.col, tw.offset, tw.height, tw.width, tw.rotateRight()];
    [(Array2D.create 2 2 true), Yellow, (4,0), 2, 2, (Array2D.create 2 2 true)]
    ]

let test_wb = 
    if wb_test_set_tetromino[0] = wb_test_set_tetromino[1] then
        printfn "Alle metoder virker"
    else
        printfn "Minimum 1 metode virker ikke"
printfn "%A" (test_wb)

let tw2 = board(35,30)
let wb_test_set_board = [
    [tw2.width, tw2.height, tw2.board, tb2_np() ,tw2.current, tw2.put(square()), tw2.take(), tw2.isValid((square(),(1,1)))];
    [35, 30, (Array2D.create 35 30 None), true , None, true, tw2.current, true]
    ]
